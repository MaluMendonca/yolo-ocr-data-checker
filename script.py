...
target_dates = ['251124', '210925']  # Datas de produção e validade ** modifque conforme necessário, mas essa é a certa para o video
target_code = '2024330'  # Codigo do lote **
        ...
        try: 
            ...
            if text and text != '0000' and text.strip(): 
                ...
                if image_fail < 3:  # Sucesso se pelo menos uma condição estiver correta 
                    ...
                    print(f' ! SUCESSO - Imagem #{successful_images}') 
                    print(f'   Data 1: {target_dates[0]} - {"ok" if date1_match else "X"}') 
                    print(f'   Data 2: {target_dates[1]} - {"ok" if date2_match else "X"}') 
                    print(f'   Lote: {target_code} - {"ok" if lote_match else "X"}') 
                    print(f'   Frame completo salvo: {success_frame_filename}') 
                    print( 
                    ...
                    print(f'  FALHA - Processando imagem: {image_filename}') 
                    print(f'   Texto reconhecido: {text}') 
                    print(f'   Sequência numérica extraída: {numeric_sequence}') 
                    print(f'   Data 1: {target_dates[0]} - {"ok" if date1_match else "X"}') 
                    print(f'   Data 2: {target_dates[1]} - {"ok" if date2_match else "X"}') 
                    print(f'   Lote: {target_code} - {"ok" if lote_match else "X"}') 
                    print('   Resultado: Falha em todas as condições.') 
                    print('\n') 
 
            else: 
                # OCR não encontrou texto válido 
                print(f'  FALHA: Nenhum texto detectado - {image_filename}') 
...
try: 
    ...
    print(f"   ❌ TOTAL Erros Data 1 ({target_dates[0]}): {date1_total_errors}") 
    print(f"   ❌ TOTAL Erros Data 2 ({target_dates[1]}): {date2_total_errors}") 
    print(f"   ❌ TOTAL Erros Lote ({target_code}): {lote_total_errors}") 
    print(f"   Objetos detectados: {len(track_ids)}") 
